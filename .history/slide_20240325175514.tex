...
\begin{frame}{Required from spectral theorem}
    \begin{itemize}
        \item 谱定理(spectral theorem)给出了对称矩阵、厄米矩阵、酉矩阵和正规矩阵等各种类型矩阵的对角化方法。谱定理的不同版本适用于不同类型的矩阵。
        \item 谱分解常见有两种形式：
                \begin{align*}
                    M = U \bigwedge U^* \\
                    M = U \bigwedge U^T
                \end{align*}
        \item 对于厄密特矩阵(Hermitian Matrix)的谱分解来说，其形式为：$M = U \bigwedge U^*$，其中$M$为厄密特矩阵，$U$为酉矩阵(unitary matrix)，$\bigwedge=\diag(\lambda_1, \lambda_2,\dots, \lambda_n)$
    \end{itemize}
\end{frame}
