...
\begin{frame}{四元数代数运算规则}
    \begin{itemize}
        ...
        \item \textbf{乘法}:两个四元数$A$和$B$乘积的定义是
              ...
              \begin{equation}
                  \left\{\begin{matrix}\begin{align*}\mathbf{ii}&=\mathbf{jj}=\mathbf{kk}=-1 \\\mathbf{ij}&=-\mathbf{ji}=\mathbf{k} \\\mathbf{jk}&=-\mathbf{kj}=\mathbf{i} \\\mathbf{ki}&=-\mathbf{ik}=\mathbf{j}\end{align*}\end{matrix}\right.
              \end{equation}
    \end{itemize}
\end{frame}
...
\begin{frame}{应用领域}
    ...
    \begin{itemize}
        ...
        \item 一项名为"Medical Healthcare System Based on Wireless Body Area Networks: The Importance of Anomaly Detection"的研究，由Hayder Hassaballah、Rashid Fayadh和Bushra AlHayali于2020年提出���旨在探讨在医疗WBSN系统中���用异���检测的现代框架\footfullcite{Medical_Healthcare}�������
    \end{itemize}
\end{frame}
