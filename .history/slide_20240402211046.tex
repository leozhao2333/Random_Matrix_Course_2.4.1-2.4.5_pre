...
\begin{frame}{Gaussian Symplectic Ensemble(GSE)}
    \begin{itemize}
        \item An $n\times n$ quaternion self-dual Hermitian matrix $^q\mathbf{A}_n=(x_{jk})_{n\times n}$ is a matrix whose entries $x_j(j,k=1,\ldots,n)$ are quaternions and satisfy $x_{jk}=\bar{x}_{kj}.$ If each $x_{jk}$ is viewed as a $2\times2$ matrix, then $\mathbf{A}_n=(x_{jk})$ is in fact a $2n\times2n$ Hermitian matrix. And, the entries of $\mathbf{A}_n$ can be represented as
    \end{itemize}
\end{frame}
...
\begin{frame}{应用领域}
    ...
    \begin{itemize}
        ...
        \item 一篇题���"Cybersecurity Vital Signs: The Role of Anomaly Detection on Insider Threat Triage"的研究��由Karla Clarke和Yair Levy�����2019年提����，探讨���异常检测在��部威胁检��中��������要作用\footfullcite{Clarke2019CybersecurityVS}。
