...
\begin{frame}{$2\times 2$Hermitian Matrix}
    \begin{itemize}
        \scriptsize
        \item Consider a $2\times 2$ real symmetric matrix $M=\begin{bmatrix}a & b+ci \\b-ci & d\end{bmatrix}$with distinct eigenvalues $\lambda_1\neq\lambda_2$. Then $M=U\begin{bmatrix}\lambda_1&0\\0&\lambda_2\end{bmatrix}U^*$, and without loss of generality we can assume $U=\begin{bmatrix}\cos\theta & -e^{-i\chi}\sin\theta\\ \left(\cos\chi+i\sin\chi\right)\sin\theta & \cos\theta\end{bmatrix}$ for some $0\leq\theta\leq\frac12\pi$ and $\quad0\leq\chi<2\pi$.\footfullcite{howard2012diagonalization}
    \end{itemize}
\end{frame}
